...
When("I send a PATCH request to {string} with the status ARRIVED") do |endpoint|
...
Then("the response status arrive booking code should be {int}") do |status_code|
  expect(@response.code).to eq(status_code)
end

Then("the response body should contain the updated status {string}") do |status|
